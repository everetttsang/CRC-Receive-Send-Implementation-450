...
string checkCRC(string msg, string generator){
	string crc="0";
  ...
  if (myfile.is_open())
  {
    while ( getline (myfile,line) )
    {
      ...
      ss << line;
			...
			cout << "Remaining Message: "<< checkCRC(message, generator) <<endl;
      if ( bin2Dec(checkCRC(message,generator)) != 0){
        cout << "Not pass bitch"<< endl;
      }
      else{
